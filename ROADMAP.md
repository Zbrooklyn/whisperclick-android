...
| 18 | **Selective text rewrite** — Select a specific sentence or phrase to rewrite instead of the entire text. Long-press or highlight text in the input field, then rewrite only that selection. Remaining text stays untouched | HIGH | TODO | Essential for longer dictations where only one part needs fixing |
| 19 | ~~Variant labels~~ — Each rewrite card shows a short description below the style name (e.g. Clean → "Grammar & punctuation fix", Professional → "Formal tone"). Makes it immediately clear which card fixes grammar vs. changes style | MEDIUM | DONE | Small subtitle under each card title |
| 20 | ~~Pager swipe jitter fix~~ — Moved height interpolation from composition phase to layout phase. Height was recalculated via `lerp()` on every frame causing full recomposition. Now uses custom `Modifier.layout` to read pager scroll state during layout only | MEDIUM | DONE | Eliminates jank when swiping between keyboard/rewrite/clipboard pages |
| 21 | **Transcription history** — Log every dictation with timestamp, original text, and which model was used (local/cloud). Accessible from TWO places: (1) **Keyboard** — 4th pager page with edit toolbar button for quick access (same pattern as clipboard history), tap to paste, long-press to re-send to Magic Rewrite; (2) **Settings app** — full scrollable history screen with search, bulk delete, and export. Persisted in Room or SharedPreferences. Useful for retrieving something you said earlier that got overwritten or dismissed | MEDIUM | TODO | Similar architecture to clipboard history but captures Whisper output instead of system clipboard. Two entry points ensure it's usable from anywhere |
